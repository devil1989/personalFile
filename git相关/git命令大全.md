使用github或者gitlab的时候，都要添加ssh的key，这个key是git软件里面创建的，把git软件里面创建的key放到github和gitlab上，就相当于通行证，可以操作代码仓库了
Ubuntu仓库下添加ssh，需要进入.ssh文件夹 cd ~/.ssh 然后，执行ssh-keygen -t rsa -C 自己的邮箱@qq.com   来生成ssh密钥，然后vim进入id_rsa.pub这个文件复制内容，把公钥放添加到gitlab或github【登录github或gitlab站点，里面有添加ssh的地方】
...
git reflog
    ...
    Git reset HEAD@{5} 就是返回到之前的那个对应的头部！！！！！！！！！！！！！
...
5.解决冲突（
    ...
    5.3：git merge-base "自己分支" master  ：找到自己分支和master的共同祖先
    
    5.4：git reset 上一部获取的分支id        ：返回到共同祖先，但是后续自己的分支的提交内容，仍然在自己的文件里面
    
    5.5：git add .    ：5.5和5.6可以合并为git ci -am “”
    
    5.6：git ci -m "note msg"   :从5.3到5.6这个行为，只是把本地分支的所有提交，重新撤销然后再统一提交到一个文件里而已；这样可以避免rebase的时候出现反复的冲突修复；
因为rebase的时候，每一个提交，都有对应的修复，比如第一个提交添加了alert(0);第二个提交又删除了alert(0),反反复复折10遍，直接用rebase需要该很多遍
...
git commit --amend命令修改提交note后，如何退出vim编辑器：
按Esc，输入:wq!
